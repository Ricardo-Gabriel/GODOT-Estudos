...
func attack() -> void:
	var randon: int = randi_range(1,2)
	if is_attacking:
		return
	
	
	animation_player.play("attack_side_" + str(randon))
	...
	if is_attacking:
		...
		if Input.is_action_pressed("move_up"):
			animation_player.play("attack_up_" + str(randon))
			
		if Input.is_action_pressed("move_down"):
			animation_player.play("attack_down_" + str(randon))
